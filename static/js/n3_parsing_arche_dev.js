        ...
        if (localStorage.getItem(project_title.replace(".xsl","")) == null) {
            ...
            downloader(host, resourceId, format, readMode, function(rs) {
                ...
                if (arche2.length !== 0) {
                    ...
                    var tbody = createBody(arche, arche2);
                } else {
                    var thead = `<thead>
                                    <tr>
...
function createBody2(arche) {
    ...
    var tbody = tbodyOpen.concat(tbodyClose);
    return tbody;
    // for (var i = 0; i < arche.length; i++) {
    //     var dl_id = "resource_" + Math.ceil(Math.random() * 10) + Math.ceil(Math.random() * 10) +Math.ceil(Math.random() * 10);
    //     tbodyOpen += `<tr>
    //     <td><a class="load_resources fade" id="${dl_id}" title="${arche[i].object}" data-target="${arche[i].subject}"  href="${'#liveToast_anchor_' + dl_id}"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-arrow-down-circle" viewBox="0 0 16 16">
    //     <path fill-rule="evenodd" d="M1 8a7 7 0 1 0 14 0A7 7 0 0 0 1 8zm15 0A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM8.5 4.5a.5.5 0 0 0-1 0v5.793L5.354 8.146a.5.5 0 1 0-.708.708l3 3a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V4.5z"/>
    //     </svg></a></td>
    //     <td>${arche[i].object.replace(regex,'')}</td>
    //     <td><a href=${arche[i].subject.replace('/api/', browser)} target="_blank" class="body_translations" title="Öffnen">Öffnen</a></td>
    //     <td><a href=${arche[i].subject + rdfxml} target="_blank" class="body_translations" title="Öffnen">Öffnen</a></td>
    //     <td><a href=${arche[i].subject + turtle} target="_blank" class="body_translations" title="Öffnen">Öffnen</a></td>
    //     </tr>`;
    // }
};
